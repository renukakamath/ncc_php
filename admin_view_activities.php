...
<div class="container">
	<div class="row">
		<?php
		echo$q="select *,concat (student.`first_name`,student.`last_name`)as sname,concat (ncc_head.`first_name`,ncc_head.`last_name`) as tname from activities inner join student using (student_id) inner join ncc_head using (head_id)";
		$ch=select($q);
		foreach ($ch as $key) 
		{
				?>
